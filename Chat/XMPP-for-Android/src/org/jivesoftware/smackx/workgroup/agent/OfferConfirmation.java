...
public class OfferConfirmation extends IQ {
	...
	private class NotifyServicePacket extends IQ {
		...
		public String getChildElementXML() {
			return "<offer-confirmation  roomname=\"" + roomName
		...
		public IQ parseIQ(XmlPullParser parser) throws Exception {
			...
			while (!done) {
				...
				if (parser.getEventType() == XmlPullParser.START_TAG
						&& "user-jid".equals(elementName)) {
					try {
						confirmation.setUserJID(parser.nextText());
					} catch (final NumberFormatException nfe) {
					}
				} else if (parser.getEventType() == XmlPullParser.START_TAG
						&& "session-id".equals(elementName)) {
					try {
						confirmation.setSessionID(Long.valueOf(parser
								.nextText()));
					} catch (final NumberFormatException nfe) {
					}
				} else if (parser.getEventType() == XmlPullParser.END_TAG
						&& "offer-confirmation".equals(elementName)) {
					done = true;
				}
			}
	...
	public void notifyService(Connection con, String workgroup,
			String createdRoomName) {
		final NotifyServicePacket packet = new NotifyServicePacket(workgroup,
				createdRoomName);
