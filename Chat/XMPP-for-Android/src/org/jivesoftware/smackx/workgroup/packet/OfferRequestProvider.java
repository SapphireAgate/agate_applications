...
public class OfferRequestProvider implements IQProvider {
	...
	public static class OfferRequestPacket extends IQ {
		...
		public OfferRequestPacket(String userJID, String userID, int timeout,
				Map<String, ?> metaData, String sessionID, OfferContent content) {
		 ...
		 * If the offer does not contain a session ID, <tt>null</tt> will be
		 * returned.
		 * 
		...
		/**
		 * Returns the number of seconds the agent has to accept the offer
		 * before it times out.
		 * 
		...
		/**
		 * Returns the userID, which is either the same as the userJID or a
		 * special value that the user provided as part of their "join queue"
		 * request.
		 * 
		 * @return the user ID.
		 */
	...
	public IQ parseIQ(XmlPullParser parser) throws Exception {
		...
		while (!done) {
			...
			if (eventType == XmlPullParser.START_TAG) {
				...
				if ("timeout".equals(elemName)) {
					...
					final RoomInvitation invitation = (RoomInvitation) PacketParserUtils
							.parsePacketExtension(RoomInvitation.ELEMENT_NAME,
									RoomInvitation.NAMESPACE, parser);
					content = new InvitationRequest(invitation.getInviter(),
							invitation.getRoom(), invitation.getReason());
				} else if (RoomTransfer.ELEMENT_NAME.equals(elemName)) {
					final RoomTransfer transfer = (RoomTransfer) PacketParserUtils
							.parsePacketExtension(RoomTransfer.ELEMENT_NAME,
									RoomTransfer.NAMESPACE, parser);
					content = new TransferRequest(transfer.getInviter(),
							transfer.getRoom(), transfer.getReason());
				}
		...
		final OfferRequestPacket offerRequest = new OfferRequestPacket(userJID,
				userID, timeout, metaData, sessionID, content);
