...
/**
 * An IQ packet that encapsulates both types of workgroup queue status
 * notifications -- position updates, and estimated time left in the queue
 * updates.
 */
public class QueueUpdate implements PacketExtension {
		...
		public PacketExtension parseExtension(XmlPullParser parser)
			...
			while (!done) {
				...
				if (parser.getEventType() == XmlPullParser.START_TAG
						&& "position".equals(elementName)) {
					try {
						position = Integer.parseInt(parser.nextText());
					} catch (final NumberFormatException nfe) {
					}
				} else if (parser.getEventType() == XmlPullParser.START_TAG
						&& "time".equals(elementName)) {
					try {
						timeRemaining = Integer.parseInt(parser.nextText());
					} catch (final NumberFormatException nfe) {
					}
				} else if (parser.getEventType() == XmlPullParser.END_TAG
						&& "queue-status".equals(elementName)) {
					done = true;
				}
			}
	...
	/**
	 * Returns the user's position in the workgroup queue, or -1 if the value
	 * isn't set on this packet.
	 * 
	...
	/**
	 * Returns the user's estimated time left in the workgroup queue, or -1 if
	 * the value isn't set on this packet.
	 * 
