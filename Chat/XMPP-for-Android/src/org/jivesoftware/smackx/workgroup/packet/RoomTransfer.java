...
/**
 * Packet extension for
 * {@link org.jivesoftware.smackx.workgroup.agent.TransferRequest}.
 * 
...
public class RoomTransfer implements PacketExtension {
	...
	public static class Provider implements PacketExtensionProvider {
		...
		public PacketExtension parseExtension(XmlPullParser parser)
			...
			invitation.type = RoomTransfer.Type.valueOf(parser
			...
			while (!done) {
				...
				if (parser.getEventType() == XmlPullParser.START_TAG) {
					if ("session".equals(elementName)) {
						invitation.sessionID = parser.getAttributeValue("",
								"id");
	...
	public static enum Type {
		/**
		 * A user is being invited to a groupchat support session. The user
		 * could be another agent or just a regular XMPP user.
		 */
		user,
		/**
		 * Some agent of the specified queue will be invited to the groupchat
		 * support session.
		 */
		queue,
		/**
		 * Some agent of the specified workgroup will be invited to the
		 * groupchat support session.
		 */
		workgroup
	}
	...
	/**
	 * JID of the entity being invited. The entity could be another agent, user
	 * , a queue or a workgroup. In the case of a queue or a workgroup the
	 * server will select the best agent to invite.
	 */
	...
	/**
	 * Text provided by the inviter explaining the reason why the invitee is
	 * invited.
	 */
	...
	public RoomTransfer(RoomTransfer.Type type, String invitee,
	...
	public String toXML() {
		...
		buf.append(
				"<session xmlns=\"http://jivesoftware.com/protocol/workgroup\" id=\"")
				.append(sessionID).append("\"></session>");
		if (invitee != null) {
