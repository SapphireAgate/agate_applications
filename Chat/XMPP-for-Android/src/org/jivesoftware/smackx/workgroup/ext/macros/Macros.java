...
/**
 * Macros iq is responsible for handling global and personal macros in the a
 * Live Assistant Workgroup.
 */
public class Macros extends IQ {
	...
	public static class InternalProvider implements IQProvider {
		...
		public IQ parseIQ(XmlPullParser parser) throws Exception {
			final Macros macroGroup = new Macros();
			...
			while (!done) {
				...
				if (eventType == XmlPullParser.START_TAG) {
					if (parser.getName().equals("model")) {
						final String macros = parser.nextText();
						final MacroGroup group = parseMacroGroups(macros);
		...
		public MacroGroup parseMacroGroup(XmlPullParser parser)
				throws Exception {
			final MacroGroup group = new MacroGroup();
	...
	public String getChildElementXML() {
		...
		if (getPersonalMacroGroup() != null) {
			...
			buf.append(StringUtils
					.escapeForXML(getPersonalMacroGroup().toXML()));
			buf.append("</personalMacro>");
		}
