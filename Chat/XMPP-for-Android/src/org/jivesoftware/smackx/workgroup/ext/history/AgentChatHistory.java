...
/**
 * IQ provider used to retrieve individual agent information. Each chat session
 * can be mapped to one or more jids and therefore retrievable.
 */
public class AgentChatHistory extends IQ {
	...
	public static class InternalProvider implements IQProvider {
		...
		private AgentChatSession parseChatSetting(XmlPullParser parser)
				throws Exception {
			...
			while (!done) {
				...
				if ((eventType == XmlPullParser.START_TAG)
					...
					final long l = Long.valueOf(dateStr).longValue();
					date = new Date(l);
				} else if ((eventType == XmlPullParser.START_TAG)
						&& ("duration".equals(parser.getName()))) {
					duration = Long.valueOf(parser.nextText()).longValue();
				} else if ((eventType == XmlPullParser.START_TAG)
						&& ("visitorsName".equals(parser.getName()))) {
					visitorsName = parser.nextText();
				} else if ((eventType == XmlPullParser.START_TAG)
						&& ("visitorsEmail".equals(parser.getName()))) {
					visitorsEmail = parser.nextText();
				} else if ((eventType == XmlPullParser.START_TAG)
						&& ("sessionID".equals(parser.getName()))) {
					sessionID = parser.nextText();
				} else if ((eventType == XmlPullParser.START_TAG)
						&& ("question".equals(parser.getName()))) {
					question = parser.nextText();
				} else if (eventType == XmlPullParser.END_TAG
						&& "chat-session".equals(parser.getName())) {
					done = true;
				}
			}
			return new AgentChatSession(date, duration, visitorsName,
		...
		public IQ parseIQ(XmlPullParser parser) throws Exception {
			...
			final AgentChatHistory agentChatHistory = new AgentChatHistory();
