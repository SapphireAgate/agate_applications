...
public class AdHocCommandDataProvider implements IQProvider {
	...
	public static class BadLocaleError implements PacketExtensionProvider {
		...
		public PacketExtension parseExtension(XmlPullParser parser)
				throws Exception {
			return new AdHocCommandData.SpecificError(
					AdHocCommand.SpecificErrorCondition.badLocale);
		}
	}
	...
	public static class BadPayloadError implements PacketExtensionProvider {
		...
		public PacketExtension parseExtension(XmlPullParser parser)
				throws Exception {
			return new AdHocCommandData.SpecificError(
					AdHocCommand.SpecificErrorCondition.badPayload);
		}
	}
	...
	public IQ parseIQ(XmlPullParser parser) throws Exception {
		boolean done = false;
		final AdHocCommandData adHocCommandData = new AdHocCommandData();
		final DataFormProvider dataFormProvider = new DataFormProvider();
		...
		// Action
		final String action = parser.getAttributeValue("", "action");
		if (action != null) {
			final Action realAction = AdHocCommand.Action.valueOf(action);
		...
		while (!done) {
			...
			if (eventType == XmlPullParser.START_TAG) {
				if (parser.getName().equals("actions")) {
					final String execute = parser.getAttributeValue("",
							"execute");
					if (execute != null) {
					...
					final AdHocCommandNote.Type type = AdHocCommandNote.Type
					...
					final XMPPError error = PacketParserUtils
							.parseError(parser);
