...
/**
 * RTPBridge IQ Packet used to request and retrieve a RTPBridge Candidates that
 * can be used for a Jingle Media Transmission between two parties that are
 * behind NAT. This Jingle Bridge has all the needed information to establish a
 * full UDP Channel (Send and Receive) between two parties. <i>This transport
 * method should be used only if other transport methods are not allowed. Or if
 * you want a more reliable transport.</i>
 * <p/>
...
public class RTPBridge extends IQ {
	...
	public static class Provider implements IQProvider {
		...
		public IQ parseIQ(XmlPullParser parser) throws Exception {
			...
			final RTPBridge iq = new RTPBridge();
						...
						for (int i = 0; i < parser.getAttributeCount(); i++) {
							if (parser.getAttributeName(i).equals("ip")) {
								...
								iq.setName(parser.getAttributeValue(i));
									...
									"porta")) {
								iq.setPortA(Integer.parseInt(parser
										.getAttributeValue(i)));
							} else if (parser.getAttributeName(i).equals(
									"portb")) {
								iq.setPortB(Integer.parseInt(parser
										.getAttributeValue(i)));
							}
						}
					} else if (elementName.equals("publicip")) {
						// String p = parser.getAttributeName(0);
						final int x = parser.getAttributeCount();
						for (int i = 0; i < parser.getAttributeCount(); i++) {
							if (parser.getAttributeName(i).equals("ip")) {
								iq.setIp(parser.getAttributeValue(i));
							}
	...
	static {
		ProviderManager.getInstance().addIQProvider(NAME, NAMESPACE,
				new Provider());
	}
	...
	public static String getPublicIP(Connection xmppConnection) {
		...
		final RTPBridge rtpPacket = new RTPBridge(
				RTPBridge.BridgeAction.publicip);
		rtpPacket.setTo(RTPBridge.NAME + "." + xmppConnection.getServiceName());
		rtpPacket.setType(Type.SET);

		// LOGGER.debug("Relayed to: " + candidate.getIp() + ":" +
		// candidate.getPort());

		final PacketCollector collector = xmppConnection
				.createPacketCollector(new PacketIDFilter(rtpPacket
						.getPacketID()));

		xmppConnection.sendPacket(rtpPacket);

		final RTPBridge response = (RTPBridge) collector
	...
	/**
	 * Get a new RTPBridge Candidate from the server. If a error occurs or the
	 * server don't support RTPBridge Service, null is returned.
	 * 
	 * @param connection
	 * @param sessionID
	 * @return
	...
	public static RTPBridge getRTPBridge(Connection connection, String sessionID) {
		...
		final RTPBridge rtpPacket = new RTPBridge(sessionID);
	 ...
	 * @return
	 */
	public static RTPBridge relaySession(Connection connection,
			String sessionID, String pass, TransportCandidate proxyCandidate,
	...
	public static boolean serviceAvailable(Connection connection) {
		...
		try {
			// DiscoverItems items =
			// disco.discoverItems(connection.getServiceName());
			...
			final DiscoverInfo discoInfo = disco.discoverInfo(connection
					.getServiceName());
			final Iterator iter = discoInfo.getIdentities();
			while (iter.hasNext()) {
				final DiscoverInfo.Identity identity = (DiscoverInfo.Identity) iter
						.next();
	...
	public String getChildElementXML() {
		final StringBuilder str = new StringBuilder();
		str.append("<" + ELEMENT_NAME + " xmlns='" + NAMESPACE + "' sid='")
				.append(sid).append("'>");
