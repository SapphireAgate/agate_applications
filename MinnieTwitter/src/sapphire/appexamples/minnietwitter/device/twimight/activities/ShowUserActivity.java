...
import sapphire.appexamples.minnietwitter.device.twimight.fragments.UserListFragment;
...
public class ShowUserActivity extends TwimightBaseActivity{
	...
	private void showUserInfo(){
		...
		if(isLocalUser(c.getString(c.getColumnIndex(TwitterUsers.COL_SCREENNAME)))) {
			showLocalUser();
		} else {
			showRemoteUser();
		}
		...
		if(!c.isNull(c.getColumnIndex(TwitterUsers.COL_SCREENNAME))){
			showUserTweetsButton.setOnClickListener(new OnClickListener(){
				...
				public void onClick(View v) {
					...
					c.moveToFirst();
					int index = c.getColumnIndex(TwitterUsers.COL_SCREENNAME);
					if (index != -1) {
						i.putExtra("screenname", c.getString(index));
						startActivity(i);
					}
				}
			});
	...
	private void showRemoteUser(){
		...
		Log.i(TAG,"showRemoteUser: " + c.getString(c.getColumnIndex(TwitterUsers.COL_SCREENNAME)) + "; rowId = " + rowId);
		/*
