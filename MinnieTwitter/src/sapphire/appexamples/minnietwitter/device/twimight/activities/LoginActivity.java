...
import sapphire.appexamples.minnietwitter.device.twimight.security.Obfuscator;
...
import sapphire.appexamples.minnietwitter.device.twimight.data.RevocationDBHelper;
...
import sapphire.appexamples.minnietwitter.device.twimight.util.TwimightSuggestionProvider;
...
public class LoginActivity extends AndroidSapphireActivity implements OnClickListener{
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if (StateSingleton.getInstance().getUser() != null) {
	...
	public void onClick(View view) {
		...
		case R.id.buttonLogin:
			ConnectivityManager cm = (ConnectivityManager)getSystemService(Context.CONNECTIVITY_SERVICE);
			if(cm.getActiveNetworkInfo() != null && cm.getActiveNetworkInfo().isConnected()){
				TextView screenname = (TextView) findViewById(R.id.username);
				if (!screenname.getText().toString().equals("")) {
					// disabling button
					buttonLogin.setEnabled(false);
					//new GetRequestTokenTask().execute();
// begin WITH_SAPPHIRE_AGATE
					UserManagementModule.login(screenname.getText().toString(), screenname.getText().toString());
					//System.out.println("User logged in.");
// end WITH_SAPPHIRE_AGATE
					new GetUserObjectTask().execute(screenname.getText().toString());
				}
			} else {
	...
	/* We try to get the User Object */
	private class GetUserObjectTask extends AsyncTask<String,Void,String> {
		@Override
		protected String doInBackground(String... params) {
			TwitterManager tm = (TwitterManager) getAppEntryPoint();
			UserManager um = tm.getUserManager();
			User user = um.getUser(params[0]);
			
			if (user == null)
				return null;
			
			StateSingleton.getInstance().setTwitterManager(tm);
			StateSingleton.getInstance().setUserManager(um);

			//System.out.println("Got user: " + user);
			StateSingleton.getInstance().setUser(user);
			
			// call the twitter service to /* verify the credentials */ put the user in the table
			Intent i = new Intent(TwitterService.SYNCH_ACTION);
			i.putExtra("synch_request", TwitterService.SYNCH_LOGIN);
			registerLoginReceiver();
			startService(i);
			
			return user.getUserInfo().getScreenName();
		}
		
		@Override
		protected void onPostExecute(String result) {
			if (result != null) {
				setTwitterScreenname(result, getApplicationContext());
				startTimeline(getApplicationContext());
			}
		}
	}
    ...
    */
	...
	static class PerformLogoutTask extends AsyncTask<Context,Void,Void> {
		...
		protected Void doInBackground(Context... params) {
			...
			TwitterService.setFavoritesSinceId(-1, context);
			TwitterService.setLastFavoritesUpdate(null, context);
			TwitterService.setMentionsSinceId(-1, context);
			TwitterService.setLastMentionsUpdate(null, context);
			TwitterService.setTimelineSinceId(-1, context);
			...
			// Set user to null
			StateSingleton.getInstance().setUser(null);
	...
	public static String getTwitterId() {
		return UserManagementModule.getUserName();
	}
