...
import sapphire.appexamples.minnietwitter.device.twimight.data.FriendsKeysDBHelper;
...
import sapphire.appexamples.minnietwitter.device.twimight.security.RevocationListEntry;
import sapphire.appexamples.minnietwitter.device.twimight.util.Constants;
import sapphire.appexamples.minnietwitter.device.twimight.util.EasySSLSocketFactory;
	...
	private class SynchAllTask extends AsyncTask<Void, Void, Boolean>{
		...
		protected Boolean doInBackground(Void... params) {
			...
			try {
				...
				if(!LoginActivity.getTwitterId().equals( twitterId  )){
	...
	private class RevokeTask extends AsyncTask<Void, Void, Boolean>{
		...
		protected Boolean doInBackground(Void... params) {
			...
			try {
				...
				// authentication
				String twitterId = tds.parseAuthentication();
				if(!twitterId.equals(LoginActivity.getTwitterId())){
					if (TwimightBaseActivity.D) Log.e(TAG, "Twitter ID mismatch!");
					return false;
				}
