...
import sapphire.appexamples.minnietwitter.device.twimight.activities.ShowDMListActivity;
import sapphire.appexamples.minnietwitter.device.twimight.activities.ShowDMUsersListActivity;
		...
		switch(dmUriMatcher.match(uri)){
			...
			case USERS: 
				...
				sql= "SELECT "
					...
					+ "AND "+DBOpenHelper.TABLE_USERS + "." + TwitterUsers.COL_ID+"<>"+LoginActivity.getTwitterId()+" "
		...
		switch(dmUriMatcher.match(uri)){
			case LIST_NORMAL:
				...
				if(c.getCount()>0){
					...
					c.moveToFirst();
					while(!c.isAfterLast()){
						boolean isOurs = Long.toString(c.getLong(c.getColumnIndex(DirectMessages.COL_SENDER))).equals(LoginActivity.getTwitterId());
						boolean isForUs = Long.toString(c.getLong(c.getColumnIndex(DirectMessages.COL_RECEIVER))).equals(LoginActivity.getTwitterId());
			...
			case LIST_DISASTER:
				...
				if(LoginActivity.getTwitterId().equals(values.getAsInteger(DirectMessages.COL_SENDER).toString())){
					
					if(cm.hasCertificate()){
						
						// we put the signature
						String text = values.getAsString(DirectMessages.COL_TEXT);
						String userId = LoginActivity.getTwitterId().toString();
					...
					//Is it for me?
					if (values.getAsLong(DirectMessages.COL_RECEIVER).toString().equals(LoginActivity.getTwitterId())){
...
//							values.put(DirectMessages.COL_ISVERIFIED, 0);
	...
	private int getDisasterID(ContentValues cv){
		...
		if(!cv.containsKey(DirectMessages.COL_SENDER) || (cv.getAsString(DirectMessages.COL_SENDER)==null)){
			senderId = LoginActivity.getTwitterId().toString();
		} else {
	...
	private Uri insertDM(ContentValues values){
		if(checkValues(values)){
			...
			if(rowId >= 0){				
				...
				// are we the receiver?
				if(values.containsKey(DirectMessages.COL_RECEIVER) && Long.toString(values.getAsLong(DirectMessages.COL_RECEIVER)).equals(LoginActivity.getTwitterId())){
